...
import (
	...
	"github.com/0xlover/squanchy/cmd/squanchy/database"
	"github.com/0xlover/squanchy/cmd/squanchy/store"
	"github.com/0xlover/squanchy/cmd/squanchy/utilities"
...
func main() {
	...
	r.Route(args.Prefix, func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			// With SMTP enabled
			if args.EnableSmtp {
				r.With(httprate.LimitByIP(args.ExtraLimit, time.Hour)).Get("/register/resend", handlers.RegisterResend)
				r.With(httprate.LimitByIP(args.ExtraLimit, time.Hour)).Post("/register/confirm", handlers.RegisterConfirm)
				r.With(httprate.LimitByIP(args.ExtraLimit, time.Hour)).Post("/recovery", handlers.Recovery)
				r.With(httprate.LimitByIP(args.ExtraLimit, time.Hour)).Post("/recovery/confirm", handlers.RecoveryConfirm)
			}
			// With TOTP enabled
			if args.EnableTotp {
				r.With(httprate.LimitByIP(args.ExtraLimit, time.Hour)).Post("/recovery/2fa", handlers.RecoveryTOTP)
			}
			// Documentation
			...
			// Easter egg
			if args.EasterEgg {
				r.Method("SQUANCH", "/invite", http.HandlerFunc(handlers.squanchy))
			}
		})
		// Protected
		r.Group(func(r chi.Router) {
			...
			// With TOTP enabled
			if args.EnableTotp {
				r.With(httprate.LimitByIP(args.ExtraLimit, time.Hour)).Put("/modify/2fa", handlers.Change2FA)
				r.With(httprate.LimitByIP(args.ExtraLimit, time.Hour)).Post("/modify/2fa/confirm", handlers.Change2FAConfirm)
			}
		})
	})
	...
	go func() {
		fmt.Println("Reference at", "http://"+server.Addr+args.Prefix+"/openapi")
		fmt.Println("Listening to requests on", "http://"+server.Addr+args.Prefix)
